...
function readChordFile(file, waveObj) {
  let base64data = file.data.replace('data:application/octet-stream;base64,','');
...
function readThirdChordFile(file) {
  thirdChordFile = file;
  initLoading();
...
function initWaveforms() {
  ...
  wavesurfer.on('seek', (progress) => {
    wavesurfer3.seekAndCenter(progress);
  });

  wavesurfer3.on('ready', function () {
    readChordFile(thirdChordFile, wavesurfer3);
    finishLoading();
